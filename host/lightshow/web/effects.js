...
/* Ported from firmware/plane/src/relay_logic.h.
 *
 * Relays are not automated separately -- three of the four sources are derived
 * from the effect engine, which is what keeps them from costing RC channels.
 * That is efficient and completely invisible, so the editor computes the same
 * decision and shows what a cue will switch. */

/** Brightest component of one pixel, which is what the firmware follows. */
function pixelLevel(pixels, index) {
  if (index * 3 + 2 >= pixels.length) return null;
  return Math.max(pixels[index * 3], pixels[index * 3 + 1], pixels[index * 3 + 2]);
}

/**
 * Whether a relay wants to be on, ignoring its minimum switching times.
 *
 * `show` is the zone's {cue, brightness}, `pixels` the rendered strip, and
 * `channelLevel` the decoded value of its own RC channel when it has one.
 */
function relayWants(relay, show, pixels, channelLevel = 0) {
  if (!show.cue) return false;               // cue 0 switches everything off
  switch (relay.source) {
    case 'pixel': {
      const level = pixels ? pixelLevel(pixels, relay.arg) : null;
      return level !== null && level > relay.threshold;
    }
    case 'brightness': return show.brightness > relay.threshold;
    case 'cue':        return show.cue >= relay.arg;
    case 'channel':    return channelLevel > relay.threshold;
    default:           return false;
  }
}

/** Plain German for what a relay follows, for the editor's output list. */
function relayExplains(relay) {
  switch (relay.source) {
    case 'pixel':      return `folgt Pixel ${relay.arg} ab Helligkeit ${relay.threshold}`;
    case 'brightness': return `an ab Helligkeit ${relay.threshold}`;
    case 'cue':        return `an ab Effekt ${relay.arg}`;
    case 'channel':    return `eigener RC-Kanal ${relay.arg} über ${relay.threshold}`;
    default:           return relay.source;
  }
}
